# Phase 4 Day 4: 可观测性/可解释性增强（v3 — 三审修订版）

> 日期: 2026-05-12
> 方案: 纯本地 SDK，零外部依赖
> v1 审核: 6.0/10 → v2 审核: 7.5/10 → 修复 3 blocking + 4 important issues
...
class TraceStep:
    ...
    # v3: 删除 start_time 字段（I1 修复：冗余且未使用）
...
class TraceCollector:
    ...
    def __init__(self, max_traces: int = 500, max_age_seconds: int = 3600):
        ...
        self._created_at = time.time()  # v3 N1: mermaid 缓存用
...
async def delete_trace(thread_id: str):
    ...
    async with collector._lock:
        if thread_id in collector._traces:
            del collector._traces[thread_id]
            collector._step_counters.pop(thread_id, None)
            return {"deleted": True}
    raise HTTPException(status_code=404, detail=f"Trace {thread_id} not found")
```
