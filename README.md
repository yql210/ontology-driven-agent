...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-1038%20passed-green.svg" alt="Tests">
  <img src="https://img.shields.io/badge/src-11.5K%20LOC-orange.svg" alt="LOC">
...
- 🛡️ **Butler 永续 Agent** — EventBus + Handler 模式，知识库自省 + 一致性守护 + 技能进化闭环
...
│              ↕ MCP 工具调用                          │
├────────────────────────────────────────────────────┤
│         Butler 永续 Agent (EventBus + Handler)      │
│   KnowledgeUpdateHandler / ReflectionHandler        │
│   Scheduler / ConsistencyWatcher / SkillEvolver    │
├────────────────────────────────────────────────────┤
│            LayerKG 知识引擎                          │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐         │
│  │ 提取引擎  │  │ 增量引擎  │  │ 查询引擎  │         │
...
│   ├── butler/
│   │   ├── engine.py          # Butler 引擎 (生命周期管理)
│   │   ├── event_bus.py       # EventBus 事件总线
│   │   ├── scheduler.py       # CronJob 调度器
│   │   ├── handlers/          # 事件处理器
│   │   ├── consistency/       # 一致性守护
│   │   ├── skills/            # 技能进化闭环
│   │   └── watchers/          # 仓库变更监听
...
# 配置环境变量（必须）
cp .env.example .env
# 编辑 .env，填入你的 Neo4j 密码和 LLM API Key
```

### 配置 (.env)

所有配置项使用 `LAYERKG_` 前缀的环境变量，完整列表见 `.env.example`：

```bash
# ---- 必填 ----
LAYERKG_NEO4J_URI=bolt://localhost:7687
LAYERKG_NEO4J_USER=neo4j
LAYERKG_NEO4J_PASSWORD=你的密码

# ---- Agent LLM（二选一）----
# 方案 A: DeepSeek（推荐）
LAYERKG_AGENT_LLM_MODEL=deepseek-chat
LAYERKG_AGENT_API_KEY=你的Key
LAYERKG_AGENT_BASE_URL=https://api.deepseek.com
...
| 指标 | 数值 |
|------|------|
| 后端代码 | 11,507 行 Python |
| 测试代码 | 22,079 行 |
| 前端代码 | 2,361 行 TypeScript/Vue |
| 测试用例 | 1,038 passed |
| Git 提交 | 75 commits |
| 知识图谱 | 9,609 节点 / 7,429 边（自身代码库） |
| 开发周期 | Phase 0-5, 15 天 |
...
- [x] **Phase 5** — Butler 永续 Agent + 一致性守护 + 技能进化闭环
- [ ] **Phase 6** — 语义约束 + Agent 护栏 + 性能优化 + Docker 部署
