...
class MigrationRunner:
    ...
    def __init__(self, store: GraphStore, registry: MigrationRegistry, *, lock_dir: Path | None = None) -> None:
        ...
        self._lock_dir = lock_dir or Path.home() / ".ontoagent"
    ...
    def _acquire_lock(self):
        ...
        try:
            ...
            raise SchemaMigrationError(
                "Another migration is in progress. Remove ~/.ontoagent/migrate.lock if stale."
            ) from None
        return lock_file
        ...
        try:
            ...
            if to_version == "0.0.0":
                ...
                if is_nebula(self._store):
                    # NebulaGraph requires concrete VIDs for DELETE VERTEX.
                    rows = self._store.query("MATCH (sv:SchemaVersion) RETURN id(sv) AS vid;")
                    for row in rows:
                        vid = row.get("vid")
                        if not isinstance(vid, str) or not vid:
                            raise RuntimeError("SchemaVersion VID must be a nonempty string")
                        self._store.delete_node(vid)
                else:
                    # Neo4j: DETACH DELETE
