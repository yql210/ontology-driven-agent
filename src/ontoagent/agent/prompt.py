...
部分操作受本体约束保护，express_intent 会自动触发约束校验：
- refactor: 检查目标代码是否处理敏感数据（DataAsset.sensitivity），是否在关键调用链上
- compliance_check: 检查目标代码的合规关联

如果约束不通过，express_intent 会返回阻断原因（如"处理 restricted 级数据"）。
你无需手动检查约束——调用 express_intent 后根据返回值判断即可。
