...
def _get_config() -> OntoAgentConfig:
    """获取或创建配置实例。
    ...
    if "config" not in _components:
        _components["config"] = OntoAgentConfig.from_env()
...
def _get_builder() -> OntoAgentBuilder:
    """获取或创建 OntoAgentBuilder 实例。

    Returns:
        OntoAgentBuilder 实例。
    """
    if "builder" not in _components:
        _components["builder"] = OntoAgentBuilder(_get_config())
