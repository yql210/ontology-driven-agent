...
def build(
    ...
    with OntoAgentBuilder(config) as builder:
...
def web(host: str, port: int, reload: bool) -> None:
    """启动 OntoAgent Web API Server"""
    import uvicorn

    if reload:
        uvicorn.run(
            "ontoagent.api.web.app:create_app",
            host=host,
            port=port,
            reload=True,
            factory=True,
...
def ask(question: str | None, interactive: bool) -> None:
    """向代码知识图谱提问。示例：ontoagent ask "merge_node 被谁调用" """
    import asyncio
    import uuid
