...
抽离自 ``LayerKGBuilder``，所有函数均为无状态函数，依赖通过参数显式传入。
"""
...
from layerkg.pipeline import builder_utils
...
def check_llm_available(config: LayerKGConfig) -> bool:
    """检查语义提取 LLM 服务是否可用。
...
def make_semantic_extractor(config: LayerKGConfig) -> SemanticExtractor:
...
# 重导出，供外部按子模块路径导入（如 ``from layerkg.pipeline.semantic_linker import ExtractedRelation``）
__all__ = [
    "CODE_ENTITY_TYPES",
