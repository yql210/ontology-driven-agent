...
class TestMigrationRegistry:
    ...
    def test_get_migration_path_gap(self):
        ...
        # m1 + builtins through 2.7.0. The final m3 is not contiguous after v2.7.0.
        assert len(path) == 11

    def test_get_latest_version(self):
        reg = MigrationRegistry()
        # builtin migrations now include v2.7.0
        assert reg.get_latest_version() == "2.7.0"
        reg.register(DummyMigration("2.7.0", "3.0.0"))
...
@pytest.mark.unit
def test_current_schema_version_is_2_7_0() -> None:
    """Workspace ACL persistence migration advances the schema to v2.7.0."""
    assert CURRENT_SCHEMA_VERSION == "2.7.0", f"Expected 2.7.0, got {CURRENT_SCHEMA_VERSION}"
...
def test_migration_registry_includes_capability_entry_identity_version() -> None:
    ...
    assert registry.get_latest_version() == "2.7.0"
...
def test_workspace_persistence_migration_is_registered_after_v2_4_0() -> None:
    ...
    assert [migration.version_to for migration in path] == ["2.5.0", "2.6.0", "2.7.0"]
    assert registry.get_latest_version() == "2.7.0"
...
def test_workspace_persistence_migration_runner_is_idempotent() -> None:
    ...
    with patch.object(runner, "_acquire_lock", return_value=MagicMock()), patch.object(runner, "_release_lock"):
        assert runner.run_pending() == ["2.5.0", "2.6.0", "2.7.0"]
...
@pytest.mark.unit
def test_workspace_acl_migration_is_registered_after_method_graph() -> None:
    """Workspace ACL identity follows the existing workspace and method graph migrations."""
    registry = MigrationRegistry()

    path = registry.get_migration_path("2.6.0", CURRENT_SCHEMA_VERSION)

    assert [migration.version_to for migration in path] == ["2.7.0"]
    assert registry.get_latest_version() == "2.7.0"


@pytest.mark.unit
def test_workspace_acl_migration_is_additive_and_owns_only_acl_grant_identity() -> None:
    """The ACL migration must not couple grants to service graph query metadata."""
    from ontoagent.store.migrations.v2_7_0_workspace_acl import WorkspaceAclMigration

    store = MagicMock()
    migration = WorkspaceAclMigration()

    migration.upgrade(store)

    statements = [call.args[0] for call in store.query.call_args_list]
    assert migration.version_from == "2.6.0"
    assert migration.version_to == "2.7.0"
    assert statements == [
    ...
    assert "OntoAgentServiceGraph" not in statements[0]
    assert "DELETE" not in statements[0] and "DROP" not in statements[0]
