...
def test_get_neo4j_lazy():
    ...
    with unittest.mock.patch("layerkg.mcp_server.Neo4jGraphStore") as mock_neo4j:
...
def test_semantic_search_default():
    ...
    with unittest.mock.patch("layerkg.mcp_server._get_builder", return_value=mock_builder):
...
def test_graph_query_basic():
    ...
    with unittest.mock.patch("layerkg.mcp_server._get_neo4j", return_value=mock_neo4j):
...
def test_get_context_basic():
    ...
    with (
        ...
        unittest.mock.patch("layerkg.mcp_server._get_chroma", return_value=mock_chroma),
    ):
