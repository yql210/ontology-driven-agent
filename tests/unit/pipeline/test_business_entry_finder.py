...
def _service(
...
) -> tuple[BusinessEntryFinder, Mock, Mock, Mock]:
    finder, repository, assembler = Mock(), Mock(), Mock()
    ...
    assembler.return_value = _result() if result is None else result
    return BusinessEntryFinder(finder, repository, assembler), finder, repository, assembler
...
def test_find_orchestrates_in_order_and_normalizes_repo_id() -> None:
    ...
    assembler.side_effect = lambda *a, **k: (events.append("assembler"), _result())[1]
    ...
    assembler.assert_called_once_with("repo-a", (CapabilityCandidate("cap-1", 0.25),), RepositoryLookup((), ()))
...
def test_find_requires_finder_to_return_a_list(matches: object) -> None:
    service, _, repository, assembler = _service(matches)
...
def test_find_validates_inputs_before_collaborators(
    ...
    service, finder, repository, assembler = _service()
    with pytest.raises(ValueError):
...
def test_find_requires_exact_lookup_and_result_types() -> None:
    service, _, _repository, assembler = _service([_match()], object(), _result())
    with pytest.raises(ValueError):
        service.find("repo-a", "orders")
    assembler.assert_not_called()

    service, _, _repository, assembler = _service([_match()], RepositoryLookup((), ()), object())
    with pytest.raises(ValueError):
        service.find("repo-a", "orders")


def test_find_propagates_collaborator_exceptions_and_preserves_order() -> None:
    service, finder, repository, assembler = _service([_match()])
